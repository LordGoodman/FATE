...
cd ${cwd}
source_code_dir=$(cd `dirname ${cwd}`; pwd)
...
for module in "federation" "proxy" "roll" "meta-service" "fateboard"
do
    echo "### START BUILDING ${module^^} ###"
    docker build --build-arg PREFIX=${PREFIX} --build-arg BASE_TAG=${BASE_TAG} -t ${PREFIX}/${module}:${TAG} -f ${source_code_dir}/docker-build/docker/modules/${module}/Dockerfile ${source_code_dir}
